...
- FIX the weird t1, t2 parameters, eg the current ones are for arr rather than the sequence (ADD/UNHARDCODE the parameters for mmr t1 and t2 used in "years")
...
#This function currently just calculates the BAU MMR of 2030
...
- BAU ARR FUNCTION STUFF - replace it somehow and/or fix the stuff in that function then apply changes here
...
```{r}
SDG_MMR_single_year <- function(mmr_pivotwider_tibble, year_to_calc_sdg, reference_year = 2015, global_arr) {
...
SDG_MMR_single_year(mmr_est_unrounded_pwider, 2030, 2015, global_arr)
```
...
get_arr_sdgtarget <- function(mmr_pivotwider_tibble,
  ...
  optimize(squared_diff = 0.25, interval = c(-1,1),
...
optimize(squared_diff, interval = c(-1,1),
           mmr2015_country = select(mmr_est_unrounded_pwider, `2015`),
           births2030_country = select(live_birth_projections2030, `Births`),
           nproject = 15)
...
#For optimization purposes in conjunction w/ function above
  ...
  testmmr <- select(SDG_MMR_single_year(mmr2015_country, year_to_calc_sdg = 2030, reference_year = 2015, global_arr), `SDG_projection`)
...
squared_diff <- function(global_arr, mmr_pivotwider_tibble,
   ...
   mmr2030 <- select(SDG_MMR_single_year(mmr_pivotwider_tibble, year_to_calc_sdg = 2030, reference_year = 2015, global_arr), `SDG_projection`)
...
- UNHARDCODE the stuff inside the summarize function if possible, ie see if a column name can vary so the general formula can written just once
